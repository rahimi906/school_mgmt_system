...
const addOrUpdateStudent = async (payload) => {
  ...
  try {
    const { rows } = await processDBRequest({ query, queryParams });
    return rows[0]; // <-- return the inserted student
  } catch (err) {
    console.error("DB error (addOrUpdateStudent):", err.message); // log actual DB error
    if (err.code === "23505") { // unique violation
      throw new Error("Email already exists");
    }
    throw err; // let express-async-handler handle it
  }
};
